...
def test_get_device_type_by_name_1():
    assert kernelci.lava.get_device_type_by_name("some_board", []) == None
def test_get_device_type_by_name_2():
    assert kernelci.lava.get_device_type_by_name("some_board", [], []) == None
@pytest.fixture
def device_types():
    device_types = [
        {"busy": 1, "idle": 0, "name": "am57xx-beagle-x15", "offline": 0},
        {"busy": 1, "idle": 0, "name": "beaglebone-black", "offline": 0},
    ]
    return device_types
@pytest.fixture
def aliases():
    aliases = [{"name": "x15", "device_type": "am57xx-beagle-x15"}]
    return aliases
def test_get_device_type_by_name_3(device_types, aliases):
    assert (
        kernelci.lava.get_device_type_by_name("some_board", device_types, aliases)
        == None
    )
